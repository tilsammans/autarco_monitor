...
-- Automatic conversion added for lastmonthData, lastYearData and lifetimeData production to kWh, MWh or GWh
...
function currentPower:updateValue(data) 
  ...
  self:updateProperty("unit", data.currentPowerUnit)
  self:updateProperty("log", details.peakPower .." kWp")
end
...
function lastDayData:updateValue(data) 
  ...
  self:updateProperty("unit", data.lastDayUnit)
...
function lastMonthData:updateValue(data) 
  ...
  self:updateProperty("unit", data.lastMonthUnit)
  self:updateProperty("log", "")
end
...
function QuickApp:unitCheckWatt(measurement) -- Set the measurement and unit to Watt, Kilowatt, Megawatt or Gigawatt
  ...
  if measurement > 1000000000 then
    return string.format("%.2f",measurement/1000000000),"Gigawatt"
  elseif measurement > 1000000 then
    return string.format("%.2f",measurement/1000000),"Megawatt"
  elseif measurement > 1000 then
    return string.format("%.2f",measurement/1000),"Kilowatt"
  else
    return string.format("%.2f",measurement),"Watt"
  end
end
...
function QuickApp:unitCheckWh(measurement) -- Set the measurement and unit to kWh, MWh or GWh
  ...
  elseif measurement > 1000 then
    ...
    return string.format("%.2f",measurement),"Wh"
  end
end
...
function QuickApp:updateLabels() -- Update the labels
  ...
  local labelText = ""
  labelText = labelText .."Current power: " ..data.currentPower .." " ..data.currentPowerUnit .."\n\n"
  labelText = labelText .."Peakpower: " ..details.peakPower .." kWp" .."\n"
  labelText = labelText .."Solar power: " ..data.solarPower .." Watt/m² (" ..solarM2 .." m²)" .."\n"
  labelText = labelText .."Lastday: " ..data.lastDayData .." " ..data.lastDayUnit .."\n"
  labelText = labelText .."Lastmonth: " ..data.lastMonthData .." " ..data.lastMonthUnit .."\n"
...
function QuickApp:valuesOverview() -- Get the values from json file Overview
  ...
  data.currentPower, data.currentPowerUnit = self:unitCheckWatt(tonumber(data.currentPower)) -- Set measurement and unit to Watt, Kilowatt, Megawatt og Gigawatt
  data.lastDayData = string.format("%.2f", jsonTable.overview.lastDayData.energy)
  data.lastDayData, data.lastDayUnit = self:unitCheckWh(tonumber(data.lastDayData)) -- Set measurement and unit to kWh, MWh or GWh
  data.lastMonthData = string.format("%.2f", jsonTable.overview.lastMonthData.energy)
  data.lastMonthData, data.lastMonthUnit = self:unitCheckWh(tonumber(data.lastMonthData)) -- Set measurement and unit to kWh, MWh or GWh
  data.lastYearData = string.format("%.2f", jsonTable.overview.lastYearData.energy)
  ...
  data.lifeTimeData = string.format("%.2f", jsonTable.overview.lifeTimeData.energy)
  ...
  if data.lifeTimeData_revenue ~= nil then -- lifeTimeData_revenue is not mandatory
...
function QuickApp:createVariables() -- Create all Variables 
  ...
  data = {}
  data.currentPower = "0"
  data.currentPowerUnit = "Watt"
  data.solarPower = "0" 
  data.lastDayData = "0"
  data.lastDayUnit = "Wh"
  data.lastMonthData = "0"
  data.lastMonthUnit = "Wh"
  data.lastYearData= "0"
...
function QuickApp:getQuickAppVariables() -- Get all Quickapp Variables or create them
  ...
  if siteID == "" or siteID == nil then
    siteID = "1" -- This siteID is just an example, it is not working 
